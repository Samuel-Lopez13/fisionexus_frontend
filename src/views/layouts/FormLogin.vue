        ...
        <div class="w-1/2 h-fit pb-pLogin pt-pLogin shadow-basic bg-white flex items-center pl-4 laptop:w-8/12 tablet:w-9/12 telefono:w-full telefono:h-full telefono:p-0 telefono:flex telefono:justify-center telefono:items-center telefono:flex-col telefono:gap-12">
            ...
            <form class="h-fit w-8/12 flex flex-col justify-center items-center gap-7 telefono:w-3/4 telefono:2/3 telefono:gap-6 telefono:h-auto">
                ...
                <div class="flex flex-col w-8/12 justify-center gap-3 telefono:w-full">
                    ...
                    <input v-model="username" :disabled=ver :class="{ 'bg-white': ver }" class="h-[40px] border-principal rounded-sm p-3 text-gray-600 hover:outline-0 focus:text-blue-900 focus:outline-0 focus:border-blue-500 focus:border-2"
                           placeholder="Correo electronico">
                </div>
                <div class="flex flex-col w-8/12 justify-center gap-3 telefono:w-full">
                    <label class="text-gray-600">Contraseña</label>
                    <input v-model="contrasena" :disabled=ver class="h-[40px] border-principal rounded-sm p-3 text-gray-600 hover:outline-0 focus:text-blue-900 focus:outline-0 focus:border-blue-500" type="password"
                           placeholder="Contraseña">
                </div>
                <div class="w-full flex justify-center">
                    <button type="submit" @click.prevent="login()" :disabled=ver :class="{ 'bg-pHover': ver}" class="w-8/12 h-[45px] text-1xl font-bold tracking-widest bg-principal text-white rounded-sm hover:bg-pHover telefono:w-full">
                        <div v-if="ver === false">
                            Iniciar sesión
                        </div>
