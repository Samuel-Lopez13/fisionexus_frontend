...
const selectedAreasAnterior = ref(['Cabeza'])
...
<template>
    ...
    <!--Escala del dolor-->
    <section v-if="selectedAreasAnterior.length > 0 || selectedAreasOtraVista.length > 0" class="rounded-sm border shadow mt-6">
                    ...
                    <div v-if="selectedAreas.length > 0">
                        <section v-for="area in selectedAreas" :key="(index === 0 ? 'anterior-' : 'otra-') + area"
                            ...
                            <div class="relative">
                                ...
                                <span class="absolute top-0 left-0 transform -translate-y-full bg-gray-100 px-2 py-1 text-xs rounded shadow-lg">{{ painLevels[area] }}</span>
                            </div>
